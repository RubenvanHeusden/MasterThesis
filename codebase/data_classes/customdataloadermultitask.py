from torchtext.data import TabularDataset, Field, LabelField, BucketIterator, Iterator
...
class CustomDataLoaderMultiTask:
    def __init__(self, datasplits, text_field, field_names):
        ...
        @param label_fields: list of variable length containing torchtext.data.Field Object
        for each possible label of the datapoint
    ...
    def construct_iterators(self, vectors: str, vector_cache: str, batch_size: int, device):
        ...
        iterators.append(DataIteratorMultiTask(train_iter, label_name=self.field_names))
        ...
        for x in range(len(self.datasplits[1:])):
            iter = Iterator(self.datasplits[x+1], batch_size=batch_size,
                         ...
                         sort_within_batch=False,
                         repeat=False)
            iterators.append(DataIteratorMultiTask(iter, label_name=self.field_names))
        return iterators, self.text_field.vocab.vectors
